...
__device__ __forceinline__ float nvfp4_value(uint8_t value) {
  constexpr float kValues[8] = {0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f};
  const float magnitude = kValues[value & 7];
  return value & 8 ? -magnitude : magnitude;
}

__device__ __forceinline__ float e4m3_value(uint8_t value) {
  __nv_fp8_e4m3 fp8;
  fp8.__x = value;
...
__device__ __forceinline__ int map_expert(
    int global_expert, const int32_t* expert_map, int global_experts, int local_experts) {
  if (global_expert < 0 || global_expert >= global_experts) {
    return -1;
  }
  const int local_expert = expert_map[global_expert];
  return local_expert >= 0 && local_expert < local_experts ? local_expert : -1;
}

__device__ __forceinline__ int scale_offset(int row, int block, int rows, int blocks) {
  const int padded_rows = (rows + 127) & -128;
  const int padded_blocks = (blocks + 3) & -4;
  return (row >> 7) * 512 * (padded_blocks >> 2) + (block >> 2) * 512 + 16 * (row & 31) +
      4 * ((row >> 5) & 3) + (block & 3);
}

__device__ __forceinline__ float scaled_activation(
    const uint8_t* packed, const uint8_t* scale, int element, int blocks) {
  const uint8_t byte = packed[element >> 1];
  const uint8_t nibble = element & 1 ? byte >> 4 : byte & 15;
  return nvfp4_value(nibble) * e4m3_value(scale[element >> 4]);
}

__device__ __forceinline__ float scaled_weight(
    ...
    int rows,
    int columns) {
  const uint8_t byte = packed[static_cast<int64_t>(row) * (columns >> 1) + (column >> 1)];
  const uint8_t nibble = column & 1 ? byte >> 4 : byte & 15;
  const int sf = scale_offset(row, column >> 4, rows, columns >> 4);
  return nvfp4_value(nibble) * e4m3_value(scale[sf]);
}

template <typename Load>
__device__ __forceinline__ void quantize_group(
    Load load, float global_scale, uint8_t* output, uint8_t* scale_output) {
  float values[16];
  float maximum = 0.0f;
#pragma unroll
  for (int i = 0; i < 16; ++i) {
...
constexpr int kMoeResidencyWords = 0;

__device__ __forceinline__ uint32_t shared_address(const void* pointer) {
  uint32_t address;
  asm("{ .reg .u64 value; cvta.to.shared.u64 value, %1; cvt.u32.u64 %0, value; }"
      : "=r"(address)
      : "l"(pointer));
  return address;
}
...
__device__ __forceinline__ void copy_async_16(
    uint32_t destination, const void* source, bool valid) {
...
__device__ __forceinline__ void copy_async_8(
    uint32_t destination, const void* source, bool valid) {
...
__device__ __forceinline__ void persistent_grid_barrier(
    int32_t* barriers, int phase) {
  constexpr int kBarrierPhases = 6;
  __shared__ int generation;
  __threadfence();
  __syncthreads();
  if (threadIdx.x == 0) {
    int32_t* arrivals = barriers + phase;
    int32_t* generations = barriers + kBarrierPhases + phase;
    generation = atomicAdd(generations, 0);
    const int ticket = atomicAdd(arrivals, 1);
    if (ticket == gridDim.x - 1) {
      atomicExch(arrivals, 0);
      __threadfence();
      atomicAdd(generations, 1);
    } else {
      while (atomicAdd(generations, 0) == generation) {
      }
    }
  }
  __syncthreads();
...
__device__ __forceinline__ void nvfp4_gemm_unit(
  ...
  const uint8_t* expert_scale =
      weight_scale + static_cast<int64_t>(expert) * weight_scale_stride;
  ...
  auto produce = [&](int ktile, int slot) {
    ...
    const uint8_t* weight_row =
        weight + static_cast<int64_t>(row_base + row) * (kK / 2);
    const int byte0 = global_ktile * 64 + chunk * 16;
    const int byte1 = byte0 + 32;
    copy_async_16(
        stage_address + row * 80 + chunk * 16,
        weight_row + byte0,
        byte0 < kK / 2);
    copy_async_16(
        stage_address + row * 80 + (chunk + 2) * 16,
        weight_row + byte1,
        byte1 < kK / 2);
    ...
    if (tid < 64) {
      ...
      if (pair_indices != nullptr || activation_row == 0) {
        ...
        const int pair = pair_indices == nullptr || activation_row >= rows
            ? anchor_pair
            : pair_indices[activation_row];
        const int row_index = pair / pair_divisor;
        const uint8_t* activation_source =
            activation + row_index * activation_row_stride;
        copy_async_16(
            stage_address + (kMoeAOffset + activation_row * 20) * 4 +
                activation_chunk * 16,
    ...
    if (tid < 32) {
      ...
      if (pair_indices != nullptr || scale_row == 0) {
        uint32_t scales = 0;
        if (scale_row < rows) {
          const int pair = pair_indices == nullptr
              ? anchor_pair
              : pair_indices[scale_row];
          const int row_index = pair / pair_divisor;
          const uint8_t* scale_source =
              activation_scale + row_index * scale_row_stride;
#pragma unroll
    ...
    if (tid >= 64 && tid < 128) {
      ...
      const int scale_base =
          (row_base >> 7) * 512 * (kPaddedBlocks >> 2) + global_ktile * 1024 +
          scale_block * 512 + scale_row * 16 + 4 * ((row_base >> 5) & 3);
      copy_async_8(
          stage_address + kMoeSfbOffset * 4 + scale_block * kMoeTileN * 4 +
              scale_row * (kMoeTileN / 8),
  ...
  constexpr int kPrologue =
      kLocalKTiles < kMoeStages - 1 ? kLocalKTiles : kMoeStages - 1;
#pragma unroll
...
#pragma unroll
  for (int ktile = 0; ktile < kLocalKTiles; ++ktile) {
    ...
    const uint32_t* stage =
        shared + (ktile % kMoeStages) * kMoeStageWords;
...
template <int kHidden, int kIntermediate, int kTopK>
__global__ void nvfp4_moe_scalar_kernel(Nvfp4MoeParams<kHidden, kIntermediate, kTopK> params) {
  ...
  for (int index = thread; index < pairs * (2 * kIntermediate); index += stride) {
    const int pair = index / (2 * kIntermediate);
    const int row = index % (2 * kIntermediate);
    const int token = pair / kTopK;
    const int global_expert = params.topk_ids[pair];
    const int expert = map_expert(
        global_expert, params.expert_map, params.global_experts, params.local_experts);
    float value = 0.0f;
    if (expert >= 0) {
      const uint8_t* a = params.x_q + token * (kHidden / 2);
      const uint8_t* a_sf = params.x_scale + token * kXScaleStride;
      const uint8_t* weight =
          params.w13 + static_cast<int64_t>(expert) * (2 * kIntermediate) * (kHidden / 2);
      const uint8_t* weight_sf =
          params.w13_scale + static_cast<int64_t>(expert) * params.w13_scale_stride;
      float accumulator = 0.0f;
#pragma unroll 4
      for (int column = 0; column < kHidden; ++column) {
        accumulator += scaled_activation(a, a_sf, column, kHidden / 16) *
            scaled_weight(weight, weight_sf, row, column, 2 * kIntermediate, kHidden);
      }
      const float alpha = row < kIntermediate ? params.g1_alpha_up[expert] : params.g1_alpha[expert];
      value = accumulator * alpha;
    }
    params.fc1[index] = value;
  }
  cg::this_grid().sync();
  ...
  for (int group = thread; group < pairs * (kIntermediate / 16); group += stride) {
    ...
    const int expert = map_expert(
        params.topk_ids[pair], params.expert_map, params.global_experts, params.local_experts);
    if (expert < 0) {
    ...
    quantize_group(
        [&](int lane) {
          const float up = fc1[column + lane];
          const float gate = fc1[kIntermediate + column + lane];
...
#pragma unroll
    for (int slot = 0; slot < kTopK; ++slot) {
      ...
      const uint8_t* a = params.act_q + pair * (kIntermediate / 2);
      const uint8_t* a_sf = params.act_scale + pair * kActScaleStride;
      const uint8_t* weight =
          params.w2 + static_cast<int64_t>(expert) * kHidden * (kIntermediate / 2);
      const uint8_t* weight_sf =
          params.w2_scale + static_cast<int64_t>(expert) * params.w2_scale_stride;
      float accumulator = 0.0f;
#pragma unroll 4
      for (int column = 0; column < kIntermediate; ++column) {
        accumulator += scaled_activation(a, a_sf, column, kIntermediate / 16) *
            scaled_weight(weight, weight_sf, row, column, kHidden, kIntermediate);
      }
      routed += params.topk_weights[pair] * params.g2_alpha[expert] * accumulator;
    }
    params.output[index] = __float2bfloat16(routed);
  }
}

template <int kHidden, int kIntermediate, int kTopK>
__global__ __launch_bounds__(kMoeThreads) void nvfp4_moe_omma_kernel(
    Nvfp4MoeParams<kHidden, kIntermediate, kTopK> params) {
  ...
  if (params.tokens < 8) {
    ...
    for (int index = threadIdx.x;
         index < kMoeStages * kDirectZeroWords;
         index += blockDim.x) {
  ...
  persistent_grid_barrier(params.barriers, 0);
  if (params.tokens >= 8) {
    for (int pair = thread; pair < pairs; pair += stride) {
      const int expert = params.pair_experts[pair];
      if (expert >= 0) {
    ...
    persistent_grid_barrier(params.barriers, 4);
    for (int expert = thread; expert < params.local_experts; expert += stride) {
    ...
    persistent_grid_barrier(params.barriers, 5);
  }
  ...
  for (int unit = blockIdx.x;
       unit < fc1_groups * kFc1Tiles * kFc1Splits;
       unit += gridDim.x) {
  ...
  persistent_grid_barrier(params.barriers, 1);
  ...
  for (int group = thread; group < pairs * (kIntermediate / 16); group += stride) {
    ...
    quantize_group(
        [&](int lane) {
          ...
          const float gate =
              fc1[kIntermediate + column + lane] +
              fc1_split[kIntermediate + column + lane];
          const float activated = up * gate / (1.0f + expf(-gate));
          return __bfloat162float(__float2bfloat16(activated));
        },
        *params.input_scale_2,
  ...
  persistent_grid_barrier(params.barriers, 2);
  ...
  persistent_grid_barrier(params.barriers, 3);
...
struct Nvfp4MoeKernel {
  static void run(
    ...
    RuntimeCheck(x.size(0) <= output.size(0), "workspace is too small");
    ...
    RuntimeCheck(expert_map.dim() == 1, "expert_map must be one dimensional");
    ...
    RuntimeCheck(
        group_pairs.numel() >= w13.size(0) * pairs,
        "group pair workspace is too small");
    RuntimeCheck(barriers.numel() >= 12, "barrier workspace is too small");
    RuntimeCheck(
        fc1_split.numel() >= pairs * 2 * kIntermediate,
        "FC1 split workspace is too small");
    ...
    Params params{
        ...
        static_cast<int32_t*>(barriers.data_ptr()),
        static_cast<int>(x.size(0)),
        static_cast<int>(expert_map.size(0)),
        static_cast<int>(w13.size(0)),
        static_cast<int>(w13_scale.numel() / w13.size(0)),
        static_cast<int>(w2_scale.numel() / w2.size(0)),
    };
    ...
    constexpr int kSharedBytes =
        (kMoeStages * kMoeStageWords + kMoeResidencyWords) * sizeof(uint32_t);
    static const cudaError_t shared_memory_status = cudaFuncSetAttribute(
        nvfp4_moe_omma_kernel<kHidden, kIntermediate, kTopK>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        kSharedBytes);
    RuntimeCheck(
        shared_memory_status == cudaSuccess,
        "NVFP4 MoE shared memory setup failed: ",
        cudaGetErrorString(shared_memory_status));
    const int blocks = std::min(work, multiprocessors);
    RuntimeCheck(blocks > 0, "empty NVFP4 MoE launch");

    const cudaStream_t stream = LaunchKernel::resolve_device(x.device());
    nvfp4_moe_omma_kernel<kHidden, kIntermediate, kTopK>
        <<<blocks, kThreads, kSharedBytes, stream>>>(params);
    const cudaError_t status = cudaGetLastError();
    RuntimeCheck(status == cudaSuccess, "NVFP4 MoE launch failed: ", cudaGetErrorString(status));
  }
};
