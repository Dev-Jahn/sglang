...
def _is_ple_cuda_graph_capturing() -> bool:
    if get_is_model_capture_mode():
        return True
...
def _prepare_ple_batch(
    ...
    *,
    ...
    mask_invalid_tokens: bool = False,
) -> Optional[_PLEBatch]:
    ...
    if ngram_size is not None:
        ...
        if use_decode_fast_path:
            # Decode rows and tokens are one-to-one. Disk graph padding is
            # excluded later through valid_tokens and the lookup sentinel.
...
class Qwen4ExpDiskEmbedding(VocabParallelEmbedding):
    ...
    def __init__(
        ...
        if embedding.weight.dtype != torch.float8_e4m3fn:
            raise TypeError(
                "--ple-storage disk requires float8_e4m3fn "
        ...
        for name in _PLE_EMBEDDING_ATTRIBUTES:
            setattr(self, name, getattr(embedding, name))
        # This subclass uses the base embedding's loader metadata and overrides
        # its gather and forward paths, so no quantization method is invoked.
        self.quant_method = None
        ...
        self._builder_args = {
            ...
            "cleanup_generations": bool(config.ple_disk_cleanup_generations),
        }
    ...
    def _fetch_to_device(
        ...
        with torch.cuda.stream(stream):
            ...
            output.copy_(raw_device.view(torch.float8_e4m3fn))
    ...
    def gather(
        ...
        *,
        ...
        if _is_ple_cuda_graph_capturing():
            return output
    ...
    def stats_snapshot(self) -> dict:
        ...
        dynamic = getattr(fetcher, "dynamic", None) if fetcher is not None else None
        if dynamic is None:
            snapshot["dynamic_admission_dropped"] = 0
            snapshot["dynamic_admission_queue_depth"] = 0
        else:
            with dynamic._lock:
...
class Qwen4ExpPLELayer(nn.Module):
    def __init__(
        ...
        if ple_storage == "disk" and self._graph_lookup_validation_interval < 0:
            raise ValueError(
                "SGLANG_PLE_DISK_GRAPH_LOOKUP_VALIDATION_INTERVAL must be "
                "non-negative"
            )
        self._graph_embedding_snapshot_buffers = {}
        self._graph_staging_digest_buffers = {}
        self._graph_staging_expected_buffers = {}
        self._graph_staging_mismatch_buffers = {}
        self._graph_staging_result_buffers = {}
        self._pending_graph_staging_validation = None
        self._completed_graph_staging_validation = deque()
        ...
        self._graph_replay_steps = defaultdict(int)
    ...
    def prepare_cuda_graph_prefetch_buffer(
        ...
        """Allocate the fixed disk graph output before capture begins."""
        if self._prefetch_stream is None or lookup_tokens <= 0:
            return
        if not isinstance(self.ple_embedding.ngram_embedding, Qwen4ExpDiskEmbedding):
            return
    ...
    def reset_cuda_graph_capture_buffers(self) -> None:
        buffers = getattr(self, "_graph_prefetch_buffers", None)
        if buffers is None:
            self._graph_prefetch_buffers = {}
        else:
            buffers.clear()
        ...
        for name in (
            "_graph_staging_digest_buffers",
            "_graph_staging_expected_buffers",
            "_graph_staging_mismatch_buffers",
            "_graph_staging_result_buffers",
        ):
    ...
    def _is_capturing() -> bool:
        ...
        The model-capture flag is runner-set and not every graph runner
        ...
        driver as well.
        """
        return _is_ple_cuda_graph_capturing()
    ...
    def _get_prefetch_buffer(
        ...
        if self._is_capturing():
            ...
            return self._select_graph_prefetch_buffer(
                graph_key, device=lookup_ids.device, allocate=True
            )
    ...
    def _select_graph_prefetch_buffer(
        ...
        *,
        ...
        if isinstance(self.ple_embedding.ngram_embedding, Qwen4ExpDiskEmbedding):
            if self._graph_prefetch_buffer is None:
                raise RuntimeError(
                    "PLE disk graph staging must be preallocated and zeroed "
                    "before capture"
                )
            if not allocate and graph_key not in self._graph_lookup_id_buffers:
                raise RuntimeError(
                    "PLE graph replay has no captured staging buffer for "
                    f"{graph_key[0].name} with {lookup_tokens} lookup tokens"
                )
            buffer = self._graph_prefetch_buffer
            if buffer.shape[0] < lookup_tokens:
                raise RuntimeError(
    ...
    def start_prefetch(
        ...
        prefetched = self._get_prefetch_buffer(
            lookup_tokens,
            lookup_ids,
            graph_key if capturing_disk else None,
        )
        ...
        if capturing_disk:
            ...
            if owner is not None and runner_graph_key not in (None, owner):
                raise RuntimeError(
                    "PLE graph capture key is already owned by another graph: "
                    f"{graph_key}"
                )
            self._graph_lookup_id_buffers[graph_key] = lookup_ids
            if graph_key not in self._graph_lookup_id_buffer_owners or owner is None:
    ...
    def _graph_lookup_validation_required(
        ...
        required = graph_key in self._graph_lookup_validation_due
        if interval > 0:
            required = required or replay_step % interval == 0
        return required

    def _next_graph_replay_step(
        self,
        graph_key: Tuple[ForwardMode, int, Optional[str], Optional[str]],
    ) -> int:
        steps = self._graph_replay_steps
        if not isinstance(steps, dict):
            steps = self._graph_replay_steps = defaultdict(int)
        steps[graph_key] += 1
        return steps[graph_key]
    ...
    def prepare_cuda_graph_replay(
        ...
        lookup_tokens = lookup_ids.shape[0]
        if lookup_tokens == 0:
            raise RuntimeError("PLE disk graph replay has zero lookup rows")
        if graph_key is None:
            raise RuntimeError("PLE graph replay key is missing")
        replay_step = self._next_graph_replay_step(graph_key)
        self._graph_replay_step_index = replay_step
        validate_lookup_ids = self._graph_lookup_validation_required(
            graph_key, replay_step
        )
        if validate_lookup_ids:
            self._graph_lookup_validation_due.discard(graph_key)
            self._pending_graph_lookup_validation = (
                graph_key,
                replay_step,
                lookup_ids.detach().clone(),
            )
        prefetched = self._select_graph_prefetch_buffer(graph_key)
        output_view = prefetched.view(lookup_tokens, self.ple_embedding.ngram_heads, -1)
        stream = self._prefetch_stream
        stream.wait_stream(torch.cuda.current_stream())
        lookup_ids.record_stream(stream)
    ...
    def finish_cuda_graph_replay(self) -> None:
        self._consume_graph_validation(
            "_completed_graph_staging_validation",
            "PLE graph staged rows differ at consumption",
            limit=1,
        )
        self._consume_graph_validation(
        ...
        if self._pending_graph_embedding_validation is not None:
            ...
            self._pending_graph_embedding_validation = None
        pending_staging = getattr(self, "_pending_graph_staging_validation", None)
        if pending_staging is not None:
            graph_key, replay_step = pending_staging
            actual = getattr(self, "_graph_staging_digest_buffers", {}).get(graph_key)
            expected = getattr(self, "_graph_staging_expected_buffers", {}).get(
                graph_key
            )
            if actual is None or expected is None:
                self._pending_graph_staging_validation = None
                raise RuntimeError(
                    "PLE graph staging digest buffer is missing for %s with "
                    "%d lookup tokens at replay step %d"
                    % (graph_key[0].name, graph_key[1], replay_step)
                )
            row_mismatch = getattr(self, "_graph_staging_mismatch_buffers", {}).get(
                graph_key
            )
            if row_mismatch is None:
                row_mismatch = torch.empty_like(actual, dtype=torch.bool)
                buffers = getattr(self, "_graph_staging_mismatch_buffers", None)
                if buffers is None:
                    buffers = self._graph_staging_mismatch_buffers = {}
                buffers[graph_key] = row_mismatch
            result = getattr(self, "_graph_staging_result_buffers", {}).get(graph_key)
            if result is None:
                result = torch.empty((), dtype=torch.bool, device=actual.device)
                buffers = getattr(self, "_graph_staging_result_buffers", None)
                if buffers is None:
                    buffers = self._graph_staging_result_buffers = {}
                buffers[graph_key] = result
            torch.ne(actual, expected, out=row_mismatch)
            torch.any(row_mismatch, out=result)
            self._record_graph_validation(
                "_completed_graph_staging_validation",
                graph_key[1],
                replay_step,
    ...
    def _record_graph_validation(
        ...
        if free_slots:
            ...
            ready = torch.cuda.Event()
        reduced = (
            mismatch.reshape(1) if mismatch.numel() == 1 else mismatch.any().reshape(1)
        )
        host_result.copy_(reduced, non_blocking=True)
        ...
        try:
            for lookup_tokens, replay_step, host_result, _ in consumed:
                if bool(host_result.numpy()[0]):
                    raise RuntimeError(
                        f"{error_message} on replay step {replay_step} "
    ...
    def wait_cuda_graph_replay(self) -> None:
        ...
        try:
            ...
            graph_key = self._graph_replay_key
            expected_buffers = getattr(self, "_graph_staging_expected_buffers", None)
            if expected_buffers is None:
                expected_buffers = self._graph_staging_expected_buffers = {}
            expected = expected_buffers.get(graph_key)
            if expected is None:
                staged_rows = (
                    self._graph_replay_lookup_tokens * self.ple_embedding.ngram_heads
                )
                expected = torch.empty(
                    (staged_rows,),
                    dtype=torch.float32,
                    device=self._graph_replay_prefetch_buffer.device,
                )
                expected_buffers[graph_key] = expected
            staged_rows = (
                self._graph_replay_lookup_tokens * self.ple_embedding.ngram_heads
            )
            torch.sum(
                self._graph_replay_prefetch_buffer.reshape(
                    -1,
                    self._graph_replay_prefetch_buffer.numel() // staged_rows,
                ),
                dim=1,
                dtype=torch.float32,
                out=expected,
            )
            self._pending_graph_staging_validation = (
                graph_key,
                self._graph_replay_step_index,
            )
            if self._validate_graph_staging:
                self._pending_graph_embedding_validation = (
                    self._graph_replay_key,
                    self._graph_replay_step_index,
    ...
    def _consume_prefetched_embeddings(
        ...
        if captured_disk:
            ...
            digest_buffers = getattr(self, "_graph_staging_digest_buffers", None)
            if digest_buffers is None:
                digest_buffers = self._graph_staging_digest_buffers = {}
            digest = digest_buffers.get(graph_key)
            staged_rows = lookup_tokens * self.ple_embedding.ngram_heads
            if digest is None:
                digest = torch.empty(
                    (staged_rows,), dtype=torch.float32, device=embeddings.device
                )
                digest_buffers[graph_key] = digest
            torch.sum(
                embeddings.reshape(-1, embeddings.numel() // staged_rows),
                dim=1,
                dtype=torch.float32,
                out=digest,
            )
            if self._validate_graph_staging:
                ...
                if snapshot is None:
                    ...
                    self._graph_embedding_snapshot_buffers[graph_key] = snapshot
...
class Qwen4ExpModel(Qwen3_5ForCausalLM):
    ...
    def __init__(
        ...
        self._ple_disk_storage = disk_storage
    ...
    def prepare_model_batch(self, schedule_batch, forward_batch: ForwardBatch) -> None:
        ...
        if forward_batch.forward_mode.is_extend_without_speculative():
            if schedule_batch is None:
                # Prebuilt forwards do not carry request token histories, so
                # this entry point cannot form prefill look-ahead contexts.
                for ple in disk_layers:
                    ple.set_disk_prefill_contexts(None)
                return
    ...
    def prewarm_cuda_graphs(
        ...
        if graph_config.prefill.backend != Backend.DISABLED:
            # Prefill graph buckets are aggregate token counts, unlike decode
            # buckets whose size is converted by max_decode_logits_rows().
            max_tokens = max(
                max_tokens,
        ...
        if max_tokens <= 0 and any(
            ...
            raise RuntimeError(
                "--ple-storage pinned cannot capture a zero-token PLE staging "
                "extent; enable a nonzero --cuda-graph-backend-prefill bucket "
                "or decode CUDA graph batch size"
            )
    ...
    def _forward_impl(
        ...
        ple_batch = (
            _prepare_ple_batch(
                ...
                ngram_size=self.ple_ngram_size,
                ngram_eos_token_id=self.ple_ngram_eos_token_id,
                mask_invalid_tokens=getattr(self, "_ple_disk_storage", False),
            )
            if self.has_ple
            else None
        )
...
class Qwen4ExpForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def _assert_ple_storage_matches_config(self) -> None:
        text_config = getattr(self.config, "text_config", self.config)
        requested = resolve_ple_storage(text_config, default="gpu")
        expected_type = {
            "gpu": VocabParallelEmbedding,
            "pinned": Qwen4ExpPinnedHostEmbedding,
            "disk": Qwen4ExpDiskEmbedding,
        }[requested]
        for name, module in self.named_modules():
            ...
            matches = (
                type(embedding) is VocabParallelEmbedding
                if requested == "gpu"
                else isinstance(embedding, expected_type)
            )
            if not matches:
                raise RuntimeError(
                    f"Qwen4 PLE storage mismatch for {name}: requested "
                    f"{requested}, constructed {type(embedding).__name__}"
                )
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        self._assert_ple_storage_matches_config()
